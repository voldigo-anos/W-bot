...
// ========== LISTE DES PERSONNAGES (version originale, sans ajouts) ==========
const characters = [
...
module.exports = {
  config: {
    ...
    role: 0,
    description: {
      en: "Naruto fighting game with advanced chakra system, 57 characters, special moves and ultimate jutsus"
    },
    category: "game",
    nixPrefix: true,
    guide: {
  ...
  onStart: async function ({ sock, chatId, args, event, senderId, reply, prefix, commandName, usersData }) {
    ...
    if (global.NixBot.narutoSessions[threadId]) {
      return reply("❌ Une partie est déjà en cours dans ce salon. Tapez `fin` pour l'arrêter.");
    }
  ...
  onReply: async function ({ sock, chatId, message, senderId, event, usersData }) {
    ...
    const data = global.NixBot.onReply.find(
      r => r.commandName === "naruto-storm" && r.messageID === repliedMsgId
    );
    if (!data) return;
    ...
    if (!session) {
      ...
      return sock.sendMessage(chatId, { text: "❌ Aucune partie en cours. Lancez /naruto-storm pour commencer." }, { quoted: event });
    }
    ...
    switch (data.type) {
      ...
      case "naruto_choose_p2": {
        ...
        if (cleanBody === "p2") {
          ...
          const p1Name = (await sock.getChatMember(chatId, session.players.p1)).user?.first_name || "Joueur 1";
          const sentMsg = await sock.sendMessage(chatId, {
            text: characterList + `\n\n@${p1Name} 𝗝𝗼𝘂𝗲𝘂𝗿 𝟭, 𝗿𝗲́𝗽𝗼𝗻𝗱𝗲𝘇 𝗮𝘃𝗲𝗰 𝗹𝗲 𝗻𝘂𝗺𝗲́𝗿𝗼 𝗱𝘂 𝗽𝗲𝗿𝘀𝗼𝗻𝗻𝗮𝗴𝗲 (1-${characters.length})`
          }, { quoted: event });
      ...
      case "naruto_choose_characters_p2": {
        ...
        const p1Name = (await sock.getChatMember(chatId, session.players.p1)).user?.first_name || "Joueur 1";
        const p2Name = (await sock.getChatMember(chatId, session.players.p2)).user?.first_name || "Joueur 2";
      ...
      case "naruto_battle": {
        ...
        async function sendBattleUpdate() {
          ...
          if (session.p1HP <= 0 || session.p2HP <= 0) {
            ...
            const nextName = (await sock.getChatMember(chatId, nextPlayer)).user?.first_name || (session.turn === "p1" ? "Joueur 1" : "Joueur 2");
